...
ggplot(res, aes(x=Nes, y=Q))+
    ...
    xlab("2*Ne*s")+
    ylab("Probability of fixation, lambda (*1/2Ne)")+
    theme_classic()
```
...
```{r inreedingsheep, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Relationship between inbreeding and survival in a feral population of Soay sheep. Survival of sheep is strongly density dependent, as indicatesd by the lower average survival of individuals at medium and high densities (right graph). Genetic analyses also revealed low to moderate levels of inbreeding, as estimated by a multi-locus coefficient of inbreeding [see Equation (6.14)] (top graph). Finally, survival at medium and high densities depends on the degree of inbreeding (left graph).  [Data](data/6_inbreeding.csv) from Pemberton et al. (2017)."}
